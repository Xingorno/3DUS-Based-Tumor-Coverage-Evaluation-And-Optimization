...
template std::array<std::array<std::vector<ray_physics::segment>, 1>, 10000> scene::cast_rays<1, 10000>(transducer_ & transducer);
        ...
        for (size_t ray_i = 0; ray_i < ray_count; ray_i++)
        {
            ...
            for(size_t sample_i=0; sample_i < sample_count; sample_i++)
            {
                ...
                // for (size_t sample_i = 0; sample_i < sample_count; sample_i++)
                {
                    ...
                    // if (!ray_.null)
                    // {
                    float r_length = 500; //[mm]
                    auto to = ray_.from + enlarge(ray_.direction, r_length); //[mm]
            
                    btCollisionWorld::ClosestRayResultCallback closestResults(ray_.from + 0.1*ray_.direction ,to); // adding ray_direction * 0.1 is for uncorrect ray tracing operation nearby the "from" point 
                    
                    m_dynamicsWorld->rayTest(ray_.from + 0.1*ray_.direction,to,closestResults);
                    ...
                    if (closestResults.hasHit())
                    {
                        ...
                        // float temp_distance = ray_.from.distance(closestResults.m_hitPointWorld);
                        // if (temp_distance < margin_threshold)
                        // {
                        ...
                        if (samples_vector[sample_i].size() > 0)
                        {
                            btVector3 FROM = samples_vector[sample_i][0].from;
                            samples_vector[sample_i].pop_back();
                            samples_vector[sample_i].emplace_back(segment{FROM, closestResults.m_hitPointWorld, ray_.direction, result.reflected_intensity, intensity_before_hit, ray_.media.attenuation, distance_before_hit, ray_.media});
                        }
                        else
                        {
                        ...
                        // margin_threshold = temp_distance;
                        // }
                        // else
                        // {
                        //     // segments_vector.emplace_back(segment{ray_.from, closestResults.m_hitPointWorld, ray_.direction, result.reflected_intensity, intensity_before_hit, ray_.media.attenuation, distance_before_hit, ray_.media});
                    ...
                    // segments_vector.emplace_back(segment{ray_.from, to, ray_.direction, 0.0f, ray_.intensity, ray_.media.attenuation, ray_.distance_traveled, ray_.media});
                        auto to_1 = ray_.from + enlarge(ray_.direction, 1000);
